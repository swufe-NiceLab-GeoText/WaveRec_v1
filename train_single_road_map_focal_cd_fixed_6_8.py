"""
WaveRec Lightweight V8 CDXA Training Script (Fixed Version)
"""
...
import PIL.ImageOps
from WaveRec_lightweight_v8_cdxa_avg import WaveRec
...
save_path = f'./masked-{args.model}-{args.dataset}-{args.n_channels}-focal_l1-lightweight-CD-fixed'
...
def choose_model():
    if args.model == 'WaveRec_road_map_v5_cd':
...
class MaskedTrainer:
    def __init__(self, args):
        ...
        self.task_records = {
            task: {
                "train_loss": [],
                "val_mse": [],
                "test_results": {"MSE": 0, "MAE": 0, "MAPE": 0, "RMSE": 0}
    ...
    def _load_roadmap(self):
        ...
        roadmap_size = args.height * args.scale_factor
        roadmap = np.expand_dims(cv2.resize(np.array(roadmap), (roadmap_size, roadmap_size)), 0)
    ...
    def test_task(self, model, test_loader, task_name):
        ...
        print(f"Test Results - MSE: {metrics['MSE']:.4f}, RMSE: {metrics['RMSE']:.4f}, MAE: {metrics['MAE']:.4f}, MAPE: {metrics['MAPE']:.2%}")
    ...
    def run(self):
        ...
        for task in self.train_sequence:
            result_dict["tasks"][task] = {
                ...
                "test_rmse": round(self.task_records[task]["test_results"]["RMSE"], 6),
                "test_mae": round(self.task_records[task]["test_results"]["MAE"], 6),
                "test_mape": round(self.task_records[task]["test_results"]["MAPE"], 6),
            }

        json_path = os.path.join(self.save_path, "train_results.json")
