"""
WaveRec Lightweight V8 CDXA Avg 
...
-  sym4 
 ...
 WaveRec_lightweight_v8_cdxa.py 
"""
...
class DWT2D(nn.Module):
    def __init__(self, wave: str = 'haar'):
        ...
        if wave == 'haar' or wave == 'db1':
            ...
            self.dec_lo = torch.tensor([
                -0.0757657147892733, -0.0296355276459985, 0.4976186676320254,
                0.8037387518092156, 0.2978577956200254, -0.0992195435769252,
                -0.0126039672620378, 0.0322231006040427
            ], dtype=torch.float32)
            self.dec_hi = torch.tensor([
                0.0322231006040427, 0.0126039672620378, -0.0992195435769252,
                -0.2978577956200254, 0.8037387518092156, -0.4976186676320254,
...
class FrequencyAvgFusion(nn.Module):
    """
    ...
    def __init__(self, channels: int):
        super(FrequencyAvgFusion, self).__init__()
...
class WaveletMultiModalFusionV8ProAvg(nn.Module):
    """
    V8 Pro Avg 

    ：
    -  FrequencyAwareFusion（+）
    -  FrequencyAvgFusion ：(LL + LH + HL + HH) / 4.0
    - ，
    """
    def __init__(self, channels: int, ext_dim: int = 12, road_channels: int = 64,
                 num_heads: int = 4, dropout: float = 0.1, wave: str = 'haar'):
        super(WaveletMultiModalFusionV8ProAvg, self).__init__()
        ...
        self.freq_avg_fusion = FrequencyAvgFusion(channels)
    ...
    def forward(self, x: torch.Tensor, ext_features: torch.Tensor, road_map: torch.Tensor) -> torch.Tensor:
        ...
        try:
            ...
            fused_freq = self.freq_avg_fusion(LL_refined, LH_refined, HL_refined, HH_refined)
...
class LightweightFeatureExtractorV8ProAvg(nn.Module):
    """
    V8 Pro Avg 

     WaveletMultiModalFusionV8ProAvg 
    """
    def __init__(self, in_channels: int = 2, out_channels: int = 64, base_channels: int = 64,
        ...
        super(LightweightFeatureExtractorV8ProAvg, self).__init__()
        ...
        if use_wavelet:
            self.wavelet_fusion = WaveletMultiModalFusionV8ProAvg(
...
class WaveRec(nn.Module):
    ...
    def __init__(self, height=16, width=16, use_exf=False, scale_factor=4,
        ...
        self.feature_extractor = LightweightFeatureExtractorV8ProAvg(
            in_channels=2,
            out_channels=64,
            base_channels=96,
            num_blocks=2,
...
if __name__ == '__main__':
    ...
    dummy_input = torch.randn(2, 2, 16, 16, device=device)
    dummy_ext = torch.randn(2, 5, device=device)
    dummy_road = torch.randn(1, 1, 64, 64, device=device)
    ...
    with torch.no_grad():
        try:
            ...
            output_pretrain, recon, mask = model(dummy_input, dummy_ext, dummy_road, is_pretrain=True)
            print(f"Pretrain forward pass successful!")
            print(f"  - Output shape: {output_pretrain.shape}")
